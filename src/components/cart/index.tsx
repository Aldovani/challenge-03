...
export function Cart({ isOpen, isOpening, onClose }: CartProps) {
  const { handleClose, isClosing, containerRef } = useCart({ onClose })
  ...
  return (
    <ReactPortal containerId="cart-container">
      {isOpen && (
        <div
          data-open={isOpening}
          className="data-[open='true']:scale-x-100 data-[open='true']:translate-x-0  duration-200 fixed  grid grid-cols-[1fr_417px] w-full h-full bg-black bg-opacity-20 z-[100] inset-0"
        >
          <div className=""></div>
          <div
            data-open={isOpening}
            ...
            ref={containerRef}
            className="data-[close='true']:animate-close-cart bg-white h-full grid grid-rows-[64px_1fr_auto] translate-x-full animate-open-cart"
          >
            <div>
